...
Zeron releases are distributed only through GitHub Releases in
...
- `zeron-<version>-linux-x86_64.tar.gz`
- `zeron-<version>-linux-aarch64.tar.gz`
- `zeron-<version>-macos-arm64.dmg`
- `zeron-<version>-macos-arm64-app.tar.gz`
- `zeron-<version>-windows-x86_64.zip`
- `zeron-<version>-windows-x86_64.exe`
- `install.sh`
- `manifest.json`
...
`ZERON_RELEASES_URL` remains an explicit mirror/test override; normal clients
...
`https://zeron.sh/releases`. Migrate them with exactly one bridge release; do
...
3. From a separately authenticated operator checkout, download the six
   **versioned** bridge binaries and `manifest.json` from that GitHub Release.
   ...
   ```bash
   tag="v<version>"
   mkdir -p /tmp/zeron-bridge && cd /tmp/zeron-bridge
   gh release download "$tag" --repo wasimysaid/Kratos \
     --pattern 'zeron-*' --pattern manifest.json
   jq -e --arg v "${tag#v}" '.version == $v' manifest.json
   jq -r '.files | to_entries[] | select(.key | startswith("zeron-")) | "\(.value.sha256)  \(.key)"' manifest.json \
     | sha256sum -c -
   for file in zeron-*; do
