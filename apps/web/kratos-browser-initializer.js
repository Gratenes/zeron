  ...
  return {
    onProgress({ current, total }) {
      if (current >= total) {
        ...
        label.textContent = "Starting Kratos…";
        return;
      }
      bar.max = total;
      bar.value = current;
      label.textContent = `Loading Kratos… ${Math.floor((current / total) * 100)}% (${megabytes(current)} of ${megabytes(total)} MB)`;
    },
    // Trunk swallows the rejection after calling this, so window error events never fire for it.
    onFailure(error) {
      console.error(error);
      loading.classList.add("failed");
      label.textContent = "Couldn't load Kratos. Reload to try again.";
    },
  };
}
