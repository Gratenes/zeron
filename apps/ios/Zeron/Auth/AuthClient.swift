...
struct PairingInvitation: Codable, Equatable {
    ...
    static func parse(_ input: String) throws -> Self {
        ...
        let code: String
        if let url = URL(string: trimmed), url.scheme == "zeron",
...
struct DeviceIdentity: Equatable {
    private static let redeemDomain = Data("zeron.peer-auth.invite-redeem.v1\0".utf8)
    private static let authDomain = Data("zeron.peer-auth.challenge.v1\0".utf8)
