...
// `zeron://pair?payload=…` links (the same payload carried by a QR code).
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 28) {
                Spacer()
                VStack(spacing: 20) {
                    ZeronMark().frame(width: 72, height: 72)
                    VStack(spacing: 6) {
                        Text("Pair this device")
                            .font(Theme.sans(26, weight: .semibold))
                            .foregroundStyle(Theme.text)
                        Text("Paste an invitation from a trusted Zeron device, or scan its QR link.")
                            .font(Theme.sans(14))
                ...
                VStack(spacing: 12) {
                    TextField("zeron://pair?payload=…", text: $invitation, axis: .vertical)
        ...
        .onOpenURL { url in
            guard url.scheme == "zeron" else { return }
...
struct ZeronMark: View {
    ...
    var body: some View {
        ZeronMarkShape().fill(color).aspectRatio(820 / 940, contentMode: .fit)
    }
}
