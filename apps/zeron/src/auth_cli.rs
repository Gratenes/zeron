...
use zeron_engine::{Engine, EngineConfig, InstanceLock};
...
async fn call(config: &EngineConfig, method: &str, params: Value) -> anyhow::Result<Value> {
    ...
    match InstanceLock::acquire(&config.data_dir) {
        Ok(_lock) => {
            ...
            let service = zeron_engine::rpc::AuthRpc::new(auth);
            match service.handle(method, params).await? {
                zeron_rpc::RpcReply::Value(value) => Ok(value),
                zeron_rpc::RpcReply::Stream(_) => anyhow::bail!("expected a unary peer operation"),
            }
        }
        Err(lock_error) => {
            let client = zeron_rpc::connect_ws(&format!("ws://127.0.0.1:{}", config.ipc_port))
                .await
...
pub async fn initialize(
    ...
    .await?;
    println!("Durable sync peer initialized. Start or restart Zeron to use its synced profile.");
    println!(
        "For non-overlapping device availability, keep this peer running on an always-on machine."
    );
    println!(
        "Use `zeron daemon install` to run it as a service, then `zeron peer invite` to pair another device."
    );
    Ok(())
}
...
pub async fn pair(config: EngineConfig, code_file: Option<&Path>) -> anyhow::Result<()> {
    ...
    println!(
        "Device paired. Restart Zeron to open the paired profile; existing local data stays local."
    );
    Ok(())
}
...
pub async fn logout(config: EngineConfig) -> anyhow::Result<()> {
    ...
    println!("Device disconnected. Restart Zeron to return to the unchanged local-only profile.");
