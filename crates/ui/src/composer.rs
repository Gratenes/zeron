...
impl Composer {
    ...
    fn on_modified_submit(&mut self, cx: &mut Context<Self>) {
        ...
        match modified_submit_target(has_content) {
            ...
            ModifiedSubmitTarget::ActivateQueueHead => self.activate_latest_queued(cx),
        }
    }
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let container = presentation::container()
            .when_some(failure, |el, message| {
                // kratos composer.tsx `Notice` (matches the transcript
        ...
        // need the full-width actions row (kratos composer-actions.tsx
        // `mustExpand = isNew || …`).
        ...
        // The pill chrome (kratos composer.tsx): `rounded-[26px] border
