...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/bin" "$TMP/fixture/payload/zeron-1.2.3-linux-aarch64"
printf '#!/bin/sh\necho zeron fixture\n' >"$TMP/fixture/payload/zeron-1.2.3-linux-aarch64/zeron"
chmod 755 "$TMP/fixture/payload/zeron-1.2.3-linux-aarch64/zeron"
tar -czf "$TMP/fixture/asset.tar.gz" -C "$TMP/fixture/payload" zeron-1.2.3-linux-aarch64
sha="$(sha256sum "$TMP/fixture/asset.tar.gz" | awk '{print $1}')"
printf '{"version":"1.2.3","files":{"zeron-1.2.3-linux-aarch64.tar.gz":{"sha256":"%s"}}}\n' "$sha" >"$TMP/fixture/manifest.json"
...
case "$url" in
  ...
  */zeron-1.2.3-linux-aarch64.tar.gz) cp "$FIXTURE_DIR/asset.tar.gz" "$out" ;;
...
HOME="$TMP/home" PATH="$TMP/bin:$PATH" FIXTURE_DIR="$TMP/fixture" CURL_LOG="$TMP/curl.log" \
  ZERON_RELEASES_URL="http://127.0.0.1:9876/releases" \
  ZERON_INSTALL_ALLOW_INSECURE_LOCALHOST=1 XDG_RUNTIME_DIR= \
  sh "$ROOT/scripts/install.sh" >/dev/null

test -x "$TMP/home/.zeron/app/1.2.3/zeron"
test "$(readlink "$TMP/home/.zeron/app/current")" = "$TMP/home/.zeron/app/1.2.3"
grep -qx 'http://127.0.0.1:9876/releases/manifest.json' "$TMP/curl.log"
grep -qx 'http://127.0.0.1:9876/releases/zeron-1.2.3-linux-aarch64.tar.gz' "$TMP/curl.log"
...
if HOME="$TMP/other-home" PATH="$TMP/bin:$PATH" FIXTURE_DIR="$TMP/fixture" CURL_LOG="$TMP/curl.log" \
  ZERON_VERSION='../bad' XDG_RUNTIME_DIR= sh "$ROOT/scripts/install.sh" >"$TMP/version.out" 2>&1; then
  echo "malformed version unexpectedly accepted" >&2
  exit 1
fi
grep -q 'invalid ZERON_VERSION' "$TMP/version.out"
test ! -s "$TMP/curl.log"
